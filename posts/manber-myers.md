...
## Stable Radix Sort
앞서 언급한 대로 비교 기반의 정렬 알고리즘을 사용하면, 접미사들을 $$\lceil\log_{2} N\rceil$$번 $$L해야 하므로 $$O(N\log^2 N)$$ 시간복잡도를 가진다. $$O(N\log N)$$ 시간복잡도를 가지는 맨버-마이어스 알고리즘의 핵심 아이디어는 안정한 기수 정렬을 통해 $$C_H$$로부터 $$C_{2H}$$를 $$O(N)$$에 계산할 수 있다는 것이다.
